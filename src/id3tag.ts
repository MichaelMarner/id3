...
export class ID3Tag {
  static async parse(handle: Reader) {
    var tags: any = {
        title: null,
        album: null,
        artist: null,
	 ...
	 */
    let buffer = await handle.read(128, handle.size - 128);
    ...
    if (
      ...
      dv.getString(3, undefined, true) !== 'ID3' ||
      dv.getUint8(3) > 4
    ) {
      processed.v2 = true;
      return process();
    ...
    buffer = await handle.read(tagSize, headerSize);
    var dv = new EnhancedDataView(buffer),
      position = 0;
