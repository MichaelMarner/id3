 ...
 */
  static parse(
    ...
    minor = minor || 0;
    major = major || 4;
    var result: { tag: string; value?: string };
	 ...
	 */
    if (header.flags[1] !== 0) {
      throw new ID3Exception('cannot read frame');
    }
 ...
 * ID3v2.2 and earlier
 */
  static parseLegacy(buffer: ArrayBuffer): {tag: string, value?: string} {
    var result: { tag: string; value?: any };
    var dv = new EnhancedDataView(buffer),
      header = {
        id: dv.getString(3),
        type: dv.getString(1),
